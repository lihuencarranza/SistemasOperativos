...
El manejo de la señal `SIGCHLD` en la shell implica llamar a `waitpid()` con la bandera WNOHANG. Esto permite a la shell verificar si algún proceso hijo ha terminado sin bloquear la ejecución. Si `waitpid()` devuelve un PID de un proceso hijo que ha terminado, la shell le muestra un mensaje al usuario notificandolo de la terminación de este proceso.
